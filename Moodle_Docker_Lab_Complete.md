...
คำตอบ:
```
Docker Image: เป็นไฟล์ต้นฉบับที่รวบรวม Code, Runtime, และ Library ต่างๆ ไว้ เปรียบเสมือน "แผ่นพิมพ์เขียว" หรือแผ่นติดตั้งโปรแกรมที่แก้ไขไม่ได้

Docker Container: เป็นร่างที่รันขึ้นมาจาก Image เปรียบเสมือน "บ้านที่สร้างเสร็จแล้ว" ซึ่งสามารถเริ่มทำงาน หยุด หรือลบออกได้ตลอดเวลา

ตัวอย่าง: Image คือแผ่นติดตั้ง Windows ส่วน Container คือคอมพิวเตอร์ที่กำลังรัน Windows เครื่องนั้นอยู่
...
moodle-docker (Moodle): ทำหน้าที่เป็น Web Server สำหรับประมวลผล PHP และรันหน้าเว็บ Moodle

postgres (Database): ทำหน้าที่เป็นฐานข้อมูลสำหรับจัดเก็บเนื้อหาวิชา, รายชื่อผู้ใช้ และคะแนนต่างๆ
...
คำตอบ:
```
Named Volumes (เช่น moodle_data, postgres_data)

ข้อดี: ข้อมูลไม่หายเมื่อลบ Container, Docker จัดการตำแหน่งที่เก็บไฟล์ให้เอง, ย้ายข้ามระบบง่าย

ข้อเสีย: เข้าถึงไฟล์โดยตรงจากเครื่อง Host (Mac) ได้ยากกว่าแบบ Bind Mount เพราะ Docker ซ่อนไว้ในโฟลเดอร์ระบบ
...
คำตอบ:
```
ทำหน้าที่สร้างวง Network เสมือนขึ้นมาเพื่อให้ Container กลุ่มเดียวกันคุยกันได้

Container สื่อสารกันผ่าน Service Name (เช่น Moodle คุยกับ DB ผ่านชื่อ db) โดยไม่ต้องระบุเลข IP จริง
...
คำตอบ:
```
ใช้กำหนด ลำดับการเริ่มทำงาน เพื่อให้มั่นใจว่า Database จะถูกรันขึ้นมาก่อนที่ตัวแอป Moodle จะเริ่มทำงาน ลดปัญหาหน้าเว็บพังเพราะหาฐานข้อมูลไม่เจอ
...
คำตอบ:
```
ต้องแก้ที่ส่วน ports: ใน Service ของ Moodle จาก - 80:80 เป็น - 9000:80
...
คำตอบ:
```
หมายถึงการระบุให้ Moodle ไปเชื่อมต่อกับ Database ที่เครื่องชื่อ db

ที่ไม่ใช้ localhost เพราะในโลกของ Docker นั้น localhost ของ Moodle คือตัวมันเอง ไม่ใช่เครื่อง Database ที่รันแยกกัน
...
คำตอบ:
```
ข้อดี: ติดตั้งเร็วมาก (แค่รันคำสั่งเดียว), สภาพแวดล้อมเหมือนกันทุกเครื่อง, ลบทิ้งได้สะอาดไม่รกเครื่อง

ข้อเสีย: กิน Memory สูงกว่าเล็กน้อย (จากที่เครื่อง Mac เราค้างไป), ต้องมีความรู้เรื่อง Docker เพิ่มเติม
...
คำตอบ:
```
วิธีตรวจสอบ: ใช้คำสั่ง docker ps ดูว่า DB รันอยู่ไหม หรือใช้ docker logs moodle-docker เพื่อดูข้อความ Error ที่เกิดขึ้น


วิธีแก้ไข: ตรวจสอบว่า MOODLE_DB_PASSWORD ตรงกันทั้งสองฝั่ง และเช็กว่าทั้งคู่รันอยู่ใน networks เดียวกันหรือไม่
...
คำตอบ:
```
Container จะถูกลบ และ Volume (ข้อมูลทั้งหมด) จะถูกลบทิ้งถาวร รวมถึงข้อมูลนักเรียนที่เราเพิ่ง Upload เข้าไปด้วย
